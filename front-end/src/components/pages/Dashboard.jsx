...
const Dashboard = ({ selectedSubjectGrade, onBackToSubjects }) => {
  ...
  const [uploadForm, setUploadForm] = useState({
    title: "",
    description: "",
    subject: selectedSubjectGrade?.subject?.id ? selectedSubjectGrade.subject.name : (selectedSubjectGrade?.subject?.value || "Maths"),
  ...
  const renderUploadTab = () => (
    <div>
      ...
      <Card>
        ...
        <Card.Body>
          {files.length === 0 ? (
            <p className="text-muted">No files uploaded yet. Upload your first study material above!</p>
          ) : (
            <ListGroup>
              {files.map((file) => (
  ...
  const renderStudyTab = () => (
    <div>
      ...
      <Card className="mb-4">
        ...
        <Card.Body>
          {commonBooks.length === 0 ? (
            <p className="text-muted">No curriculum materials available for this subject and grade.</p>
          ) : (
            <ListGroup>
              {commonBooks.map((book) => (
                <ListGroup.Item key={book.id} className="d-flex justify-content-between align-items-center">
                  <div>
                    ...
                    <small className="text-muted">
                      {book.subject} • {book.grade} • {new Date(book.uploaded_at).toLocaleDateString()}
                    </small>
                  </div>
      ...
      <Card className="mb-4">
        ...
        <Card.Body>
          {summaries.length === 0 ? (
            <p className="text-muted">No summaries generated yet. Upload a document and generate a summary!</p>
          ) : (
            <ListGroup>
              {summaries.map((summary) => (
                <ListGroup.Item key={summary.id}>
                  ...
                  <small className="text-muted">
                    {summary.subject} • {summary.grade} • {new Date(summary.created_at).toLocaleDateString()}
                  </small>
                </ListGroup.Item>
              ))}
            </ListGroup>
      ...
      <Card>
        ...
        <Card.Body>
          {quizzes.length === 0 ? (
            <p className="text-muted">No quizzes generated yet. Upload a document and generate a quiz!</p>
          ) : (
            <ListGroup>
              {quizzes.map((quiz) => (
                <ListGroup.Item key={quiz.id}>
                  ...
                  <p className="mb-2">
                    {quiz.questions.questions?.length || 0} questions • {quiz.subject} • {quiz.grade}
                  </p>
                  <small className="text-muted">
            ...
            <div className="d-flex justify-content-between align-items-center">
              <div>
                ...
                <p className="text-muted mb-0">
                  {selectedSubjectGrade?.subject?.name} • {selectedSubjectGrade?.grade?.label}
                </p>
              </div>
              ...
              {quizQuestions.length > 0 && (
                <div>
                  <div className="d-flex justify-content-between align-items-center mb-3">
                    <h6>Question {currentQuestionIndex + 1} of {quizQuestions.length}</h6>
                    <Badge bg="info">{Math.round(((currentQuestionIndex + 1) / quizQuestions.length) * 100)}% Complete</Badge>
                  </div>
                  
                  <h5 className="mb-3">{quizQuestions[currentQuestionIndex]?.question}</h5>
                  
                  <div className="quiz-options">
                    {Object.entries(quizQuestions[currentQuestionIndex]?.options || {}).map(([key, value]) => (
                      <Button
                        key={key}
                        variant={userAnswers[currentQuestionIndex] === key ? "primary" : "outline-primary"}
                ...
                return (
                  <div>
                    <div className="score-display mb-4">
                      <h2 className={`text-${score.percentage >= 70 ? 'success' : score.percentage >= 50 ? 'warning' : 'danger'}`}>
                        {score.percentage}%
                      </h2>
